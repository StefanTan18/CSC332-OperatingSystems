...
int main()
{
	...
	int semATM = semget(IPC_PRIVATE, 1, 0666 | IPC_CREAT);		// Used to represent the ATM machines
	int semMutex = semget(IPC_PRIVATE, 1, 0666 | IPC_CREAT);	// Used to deal with modifying the balance
	sem_create(semATM, 2);										// There are 2 ATM machines
	sem_create(semMutex, 1);									// Allows only 1 process to modify the balance at a time
	...
	if (pid == CHILD){
		...
		{   
			...
			P(semMutex);		// Dad process will be modifying the balance
			...
			V(semATM); 		// Dad process is done using the ATM
			V(semMutex);	// Dad process is no longer modifying the balance
	...
	{ 
		...
		if (pid == CHILD)
		{
			...
			while(flag == FALSE) 
			{
				...
				P(semATM); 		// Son 1 is using the ATM
				P(semMutex); 	// Son 1 is modifying the balance
				...
				V(semATM); 		// Son 1 is done using the ATM
				V(semMutex);	// Son 1 is done modifying the balance
		...
		//Parent Process. Fork off one more child process.
			...
			if (pid == CHILD)
			{
				...
				while(flag1 == FALSE) 
				{
					...
					P(semATM); 		// Son 2 is using the ATM
					P(semMutex); 	// Son 2 is modifying the balance
					...
					V(semATM); 		// Son 2 is done using the ATM
					V(semMutex);	// Son 2 is done modifying the balance
