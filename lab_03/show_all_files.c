...
int main() {
    ...
    if (c_pid == 0) {
        ...
        char* args_list[] = {"ls", "-a", "-l", NULL};   // arguments list for the command that will be executed
        execvp("ls", args_list);        // execute command that shows all files with their permissions, owner, size, etc.
    }
